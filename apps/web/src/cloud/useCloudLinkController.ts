...
/**
 * Drives the primary environment's managed relay link. The relay (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message = cause instanceof Error ? cause.message : "Could not update remote access.";
    const traceId = findErrorTraceId(cause);
    console.error("[t3-connect] Could not update remote access", { message, traceId, cause });
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn remote access off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in before enabling this."));
        return false;
      }
