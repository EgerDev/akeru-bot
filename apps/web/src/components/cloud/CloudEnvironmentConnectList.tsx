...
/**
 * The user's linked environments from relay discovery, each with a
...
export function CloudEnvironmentConnectRows({
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through its managed tunnel.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the linked environment.";
  ...
  if (standalone && visibleEnvironments.length === 0) {
    ...
    if (discoveryProblem !== null && !environmentsState.refreshing) {
      return (
        <div className={ITEM_ROW_CLASSNAME}>
          <p className="text-sm font-medium text-destructive">Could not load linked environments</p>
